...
stable four-character cells, file letters over pieces, and an optional
second row per rank that makes squares square on tall terminals."""

import chess

from chess_tui.calculations.board_view import (
    BOARD_WIDTH,
    ROWS_SINGLE,
    ROWS_TALL,
    board_grid,
...
def test_every_line_fits_the_fixed_width():
    ...
    assert BOARD_WIDTH == 37
    assert len(lines) == ROWS_SINGLE


def test_tall_board_adds_one_blank_row_per_rank():
    lines = board_lines(board_grid(chess.STARTING_FEN), tall=True)
    assert len(lines) == ROWS_TALL
    assert lines[2] == ""  # the filler row under rank 8
    assert lines[1] == AFTER_E4[1]  # rank 8 is untouched by 1. e4
    assert lines[-1] == AFTER_E4[-1]
